  ...
  jQuery(document).ready(function($) {
    ...
    $(".navigation > li, .stretchy-nav > li").on("click", function(e) {
      if (!$(this).hasClass("active")) {
        ...
        $(".navigation > li.active, .stretchy-nav > li.active").removeClass(
          "active"
        );
      ...
      e.preventDefault();
      if (
        $(".project-info-container").hasClass("slide-in") &&
        $(window).width() > 1024
      ) {
        setTimeout(function() {
    ...
    if (
      ...
      $("#link-" + window.location.hash.replace(/^#/, "")).length
    ) {
        ...
        $(window).on("popstate", function() {
          if (!window.userInteractionInHTMLArea) {
            if (
              ...
              $("#link-" + window.location.hash.replace(/^#/, "")).trigger(
                "click"
              );
            }
    ...
    $(".portfolio-section")
      ...
      .on("click", function(event) {
        ...
        $(".project-info-container." + selected_member + "").addClass(
          "slide-in"
        );
    ...
    $(document).on("click", ".close-project, .portfolio-overlay", function(
      event
    ) {
      event.preventDefault();
      $(".project-info-container").removeClass("slide-in");
